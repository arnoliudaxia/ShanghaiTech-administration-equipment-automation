...
def imgAddFont_up(oldimg, width, height):
    ...
    fnt = ImageFont.truetype('assets/font/msyh.ttf', up_fontsize)
    msg = '上海科技大学'
    draw.textsize(msg, font=fnt)
...
def imgAddFont_bottom(oldimg, width, height, info_num, info_name, note, caiwu):
    add_height = target_height - height
    im = Image.new("RGB", (width, add_height), (255, 255, 255))
    draw = ImageDraw.Draw(im)
    fnt = ImageFont.truetype('assets/font/msyh.ttf', bottom_fontsize)

    if isinstance(note, str) and len(note) != 0 and 'Z' in info_num:
        fnt = ImageFont.truetype('assets/font/msyh.ttf', 48)
        ...
        w, h = draw.textsize(info_num, font=fnt)
        draw.text(((width - w) / 2, contentHeight), info_num, fill="black", font=fnt)

        # info_name = info_name[:11]
        # 标签每行大概只能容纳14个字符，所以每隔14个字符加一个换行符
        # 并且由于标签的高度有限，所以最多只能显示两行
        info_name = '\n'.join([info_name[i:i + 14] for i in range(0, min(len(info_name),28), 14)])

        w, h = draw.textsize(info_name, font=fnt)
        draw.multiline_text(((width - w) / 2, contentHeight + bottom_fontsize + 5), info_name, fill='black', font=fnt, align="center",spacing=0)

        msg = caiwu
        w, h = draw.textsize(msg, font=fnt)
        draw.text(((width - w) / 2, contentHeight + bottom_fontsize * 3 + 10), msg, fill="black", font=fnt)
...
def add_margins(qr_code, datadf, i):
    ...
    qr_code = imgAddFont_bottom(qr_code, M, N, str(datadf['资产编码'][i]), datadf['资产名称'][i],
                                note=datadf['备注'][i], caiwu=caizhengzichanbianhao)
    return qr_code
...
def gen_qrcode(input_path, save_path):
    ...
    for i in range(len(datadf)):
        ...
        # 二维码中包含哪些信息读取配置文件
        with open("qrcodePrint/printTds.csv", 'r', encoding='utf-8') as f:
            tds = f.readlines()
            tds = [td.strip() for td in tds]

            info_list = tds[0].split(',')
        info = ''
